...
class ExtractAorticRegion(object):
    ...
    def get_aortic_region(self) -> None:
        ...
        '''
        self.aorta_left = self.combined_centroids[(-1-self.vertebrae_span):-1,:] + np.hstack([np.zeros(self.start_x_offset[-self.vertebrae_span:,:].shape).astype(int), self.start_x_offset[-self.vertebrae_span:,:]])
        self.aorta_right = self.combined_centroids[(-1-self.vertebrae_span):-1,:] + np.hstack([np.zeros(self.start_x_offset[-self.vertebrae_span:,:].shape).astype(int), self.end_x_offset[-self.vertebrae_span:,:]])
    ...
    def get_vertebrae_sizes(self) -> None:
        '''
        Method to determine vertebrae sizes
        '''
    ...
    def get_aortic_offsets(self) -> None:
        '''
        Method to determine the offset of the Aorta from the vertebrae based on their sizes
        '''
        vertebrae_width = np.asarray([x[1] for x in self.vertebrae_sizes]).T
        vertebrae_width = vertebrae_width[...,np.newaxis]
        self.start_x_offset = (vertebrae_width//2 + self.aorta_offset).astype(int)
        self.end_x_offset = (vertebrae_width//2 + self.aorta_width + self.aorta_offset).astype(int)
            ...
            for value in deviation_from_median:
                if value > median_distance*self.centroid_deviation_percentage:
                    ...
                    for idx in range(num_missing_vertebrae):
                        ...
                        start_y = (y_values_to_insert[idx] - self.median_H//2 - self.aorta_offset).astype(int)
                        end_y = (y_values_to_insert[idx] + self.median_H//2 + self.aorta_offset).astype(int)
                        start_x = (x_values_to_insert[idx] - self.median_W//2 - self.aorta_offset).astype(int)
