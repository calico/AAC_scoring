...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        with tf.name_scope("results"):
            ...
            for idx in range(n_class):
                where = tf.equal(tf.argmax(self.y, 3), idx)
                masked_predicter = tf.boolean_mask(tf.argmax(self.predicter, 3), where)
                masked_y = tf.boolean_mask(tf.argmax(self.y, 3), where)
                self.correct_pred = tf.equal(masked_predicter, masked_y)
                self.accuracy.append(tf.reduce_mean(tf.cast(self.correct_pred, tf.float32)))
            self.accuracy = tf.stack(self.accuracy)
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        with tf.name_scope("cost"):
            ...
            if cost_name == "cross_entropy":
                ...
                #class_weights = np.array([1, 2]).astype(np.float32)
                #class_weights = np.array([1, 2, 2]).astype(np.float32)
    ...
    def predict(self, model_path, x_test, y_test=None):
        """
        ...
        with tf.Session() as sess:
            ...
            if y_test is None:
                y_test = np.empty((x_test.shape[0], x_test.shape[1], x_test.shape[2], self.n_class))
    ...
    def predict_batch(self, model_path, x_test_list, y_test_list=None):
        ...
        :param model_path: path to the model checkpoint to restore
        :param x_test: Data list to predict on. Shape [n, nx, ny, channels]
        :returns prediction_list: The unet prediction Shape [n, px, py, labels] (px=nx-self.offset/2)
        """
        ...
        with tf.Session() as sess:
            ...
            if y_test_list is None:
                ...
                for x_test in x_test_list:
                    y_test = np.empty((x_test.shape[0], x_test.shape[1], x_test.shape[2], self.n_class))
                    prediction_list.append(np.argmax(sess.run(self.predicter, feed_dict={self.x: x_test, self.y: y_test, self.keep_prob: 1.}), 3))
                    accuracy_list.append(np.nan)
            else:
                # loop over the list of inputs and GTs
                for x_test, y_test in zip(x_test_list, y_test_list):
                    prediction = sess.run(self.predicter, feed_dict={self.x: x_test, self.y: y_test, self.keep_prob: 1.})
                    pred_shape = prediction.shape
                    prediction, accuracy = sess.run([self.predicter, self.accuracy], feed_dict={self.x: x_test, self.y: y_test, self.keep_prob: 1.})
                    prediction_list.append(np.argmax(prediction, 3))
...
#                                                                  staircase=True)
    ...
    def _initialize(self, training_iters, output_path, restore, prediction_path):
        ...
        tf.summary.tensor_summary('accuracy', self.net.accuracy)
            ...
            with tf.device('/gpu:0'):
                ...
                for epoch in range(epochs):
                    ...
                    with tf.device('/cpu:0'):
                        self.net.save(sess, save_path, epoch)

            logging.info("Optimization Finished!")

            return save_path

    def test(self, data_provider, model_dir, num_steps=None):
        """
        Lauches the test process

        Args:
        data_provider: data generator
        model_dir: path where to load checkpoints from
        """
    ...
    def output_minibatch_stats(self, sess, summary_writer, step, batch_x, batch_y):
        ...
        logging.info(
            "Iter {step}, Minibatch Loss= {loss}, Training Accuracy= {acc}, Minibatch error= {error}%".format(step=step,
                                                                                                           loss=loss,
                                                                                                           acc=acc,
                                                                                                           error=error_rate(
                                                                                                               predictions,
