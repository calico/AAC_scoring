...
class CalcBoxer:
  ...
  def __init__(self,img,modSet,
               minScoreV=0.1,maxVertAngleDeg=50,minScorePost=0.3,imageTag=""):
    """
  ...
  def _scoliosisCurve(self): #Eugene
    if not(self._img is None and len(self._vertBL)>=2):
         ...
         #base=GM.sortBoxesOnAxis(self._vertBL)
         #bBox=base[0];
         if len(tmpL):
         ...
         for b in GM.sortBoxesOnAxis(self._vertBL):
                ...
                vbp=VertebraBoxPair(bBox,b)
                refRay=GM.getRay(baseX,baseY,baseX,0)
                ray=GM.getRay(baseX,baseY,xMid,yMid)
         ...
         img_height, img_width, channels = subImg.shape

         frac_area = 0
         totalArea = img_height
         if totalArea > 0:
